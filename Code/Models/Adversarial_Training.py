...
class AdversarialTraining:
    ...
    def __init__(self, generator_class, discriminator_class, optimiser_D, optimiser_G, **kwargs):
        ...
        self.lr_scheduler = optim.lr_scheduler.MultiplicativeLR(
            self.optimiser_G, lr_lambda=lambda lr: 0.98)
        ...
        for epoch in range(self.start_epoch, self.grid['max_epochs']):
            ...
            for input, target, seq_length_input, seq_length_target in zip(input_train,
                ...
                real_labels = torch.ones(
                    self.grid['batch_size']).to(self.device)
                ...
                if optim_D:
                    ...
                    output_D, real_labels_flatten = self.discriminator.forward(
                        target.permute(1, 0), real_labels)  # discriminator needs transpose input
                    # calculate loss function on the batch of real examples
                    error_D_real = self.loss_function_D(
                        output_D, real_labels_flatten)
                    # Calculate gradient
                    ...
                    output_D_G, fake_labels_flatten = self.discriminator.forward(output_G.argmax(
                        dim=2).long().permute(1, 0), fake_labels)  # discriminator needs transpose input
                    # calculate loss function on the batch of fake examples
                    error_D_fake = self.loss_function_D(
                        output_D_G, fake_labels_flatten)
                    # calculate gradient
                ...
                for _ in range(self.grid['G_multiple']):
                    ...
                    output_D, real_labels_flatten = self.discriminator.forward(
                        output_G.argmax(dim=2).long().permute(1, 0), real_labels)
                    ...
                    seq_length_output = (output_G.argmax(
                        2) == self.grid['text_dictionary'].word2index['eos']).int().argmax(0).cpu().numpy()
                    seq_length_output += 1
                ...
                if batch % 20 == 0:
                    ...
                    for input, target, seq_length_input, seq_length_target in zip(input_val,
                        ...
                        val_loss += self.validation_loss_eval(
                            output_G, target, seq_length_target)

                        hypotheses = output_G.argmax(
                            dim=2).permute(1, 0).cpu().numpy()
                        hypotheses = [' '.join([self.grid['headline_dictionary'].index2word[index] for index in hypothesis if (
                            index != self.pad_idx) & (index != self.eos_idx)][1:]) for hypothesis in hypotheses]
                        references = [' '.join([self.grid['headline_dictionary'].index2word[index] for index in ref if (
                            index != self.pad_idx) & (index != self.eos_idx)][1:]) for ref in target.permute(1, 0).cpu().numpy()]
                        ROUGE = [self.rouge_get_scores(
                            hyp, ref) for hyp, ref in zip(hypotheses, references)]
                        self.rouge1 += ((np.array([x[0]['rouge-1']['f']
                                                   for x in ROUGE if x != 'drop']).mean() - self.rouge1) / val_batch)
                        self.rouge2 += ((np.array([x[0]['rouge-2']['f']
                                                   for x in ROUGE if x != 'drop']).mean() - self.rouge2) / val_batch)
                        self.rougeL += ((np.array([x[0]['rouge-l']['f']
                                                   for x in ROUGE if x != 'drop']).mean() - self.rougeL) / val_batch)
                        ...
                        outputs_true += sum(output_labels ==
                                            real_labels_flatten.cpu().numpy())

                        output_G = F.log_softmax(
                            output_G, dim=2).argmax(dim=2).long()
                        output_D_G, fake_labels_flatten = self.discriminator.forward(
                            output_G.permute(1, 0), fake_labels)  # discriminator needs transpose input
                        ...
                        outputs_true += sum(output_labels ==
                                            fake_labels_flatten.cpu().numpy())
                        # cleaning
                        del output_D, output_D_G
                        torch.cuda.empty_cache()

                    acc = 100 * float(outputs_true) / \
                        (2*self.n_batches_val*self.grid['batch_size'])
                    val_loss /= val_batch
    ...
    def _generate_batches(self, padded_input, input_lengths, padded_target, target_lengths):
        ...
        input_batches = np.array(
            np.split(
                padded_input[:, :(n_batches * self.grid['batch_size'])], n_batches, axis=1)
        )
        target_batches = np.array(
            np.split(
                padded_target[:, :(n_batches * self.grid['batch_size'])], n_batches, axis=1)
        )
        # Split input and target lenghts into batches as well
        input_lengths = np.array(
            np.split(
                input_lengths[:(n_batches * self.grid['batch_size'])], n_batches, axis=0)
        )
        target_lengths = np.array(
            np.split(
                target_lengths[:(n_batches * self.grid['batch_size'])], n_batches, axis=0)
        )
    ...
    def load(self):
        ...
        try:
            ...
            self.generator.model.load_state_dict(torch.load(
                "../data/Results/{}.pth".format(self.grid['model_name'])))
            self.optimiser_G.load_state_dict(torch.load(
                "../data/Results/opt_g_{}.pth".format(self.grid['model_name'])))

            # Load discriminator states and its optimizer
            self.discriminator.model.load_state_dict(torch.load(
                "../data/Results/disc_{}.pth".format(self.grid['model_name'])))
            self.optimiser_D.load_state_dict(torch.load(
                "../data/Results/opt_d_{}.pth".format(self.grid['model_name'])))

            # load startin epoch
            self.start_epoch = int(np.loadtxt(
